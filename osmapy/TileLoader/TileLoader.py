...
class TileLoader:
    ...
    def __init__(self, viewer: Any, config_id: int) -> None:
        self.name: str = config.slippy_tiles[config_id].name
        self.urls: list[str] = config.slippy_tiles[config_id].urls

        self.path_cache: Path = Path(__file__).parent / Path(f"../../cache/{self.name}")
        self.viewer: Any = viewer

        self.cache_json: dict[str, dict[str, Any]] = self.load_cache_json()

        self.queue: LifoQueue[Tile] = LifoQueue()
        self.lock: multiprocessing.Lock = multiprocessing.Lock()

        num_workers: int = min(2, multiprocessing.cpu_count())
        for _ in range(num_workers):
            Thread(target=self.worker, daemon=True).start()

    def _build_request_url(self, tile: Tile) -> str:
        osm_tile_url: str = random.choice(self.urls)
        template: Template = Template(osm_tile_url)
    ...
    def _fetch_tile_image(self, tile: Tile) -> Image.Image:
        url: str = self._build_request_url(tile)
    ...
    def worker(self) -> None:
        ...
        while True:
            tile: Tile = self.queue.get()
            try:
                image: Image.Image = self._fetch_tile_image(tile)

                with self.lock:
                    self.path_cache.mkdir(parents=True, exist_ok=True)
                    image.save(self.path_cache / f"{tile.name}.png")
                    expire_time: int = 60 * 60 * 24 * 7  # 7 days
                ...
                with self.lock:
                    ...
                    fallback: Image.Image = Image.open(self.viewer.asset_error_image)
                    fallback.save(self.path_cache / f"{tile.name}.png")

            finally:
                self.queue.task_done()

    def get_tile(self, tile: Tile) -> str | None:
        ...
        with self.lock:
            entry: dict[str, Any] | None = self.cache_json.get(name)
    ...
    def load_cache_json(self) -> dict[str, dict[str, Any]]:
        path: Path = self.path_cache / "database.json"
        if not path.is_file():
    ...
    def clear(self) -> None:
        with self.lock:
            self.cache_json.clear()
            self.save_cache_json()

    def draw(self, viewer: Any, qpainter: QPainter, alpha: float) -> None:
        qpainter.setOpacity(alpha)
        main_tile: Tile = Tile(viewer.lat, viewer.lon, viewer.zoom)
        offset_x: int = int((viewer.x - main_tile.center_x) * viewer.scale_x)
        offset_y: int = int((viewer.y - main_tile.center_y) * viewer.scale_y)
        ...
        for a in range(-num_x, num_x + 1):
            for b in range(-num_y, num_y + 1):
                tile: Tile = Tile.from_num(
                    main_tile.xtile + a, main_tile.ytile + b, main_tile.zoom
                )
                if not tile.check_existance():
                    continue

                path_image: str | None = self.get_tile(tile)
                if path_image is None:
                    pic: QPixmap = QPixmap(viewer.asset_error_image)
                else:
